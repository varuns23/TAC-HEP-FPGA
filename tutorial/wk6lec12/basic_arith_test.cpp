...
int main() {
  ...
  fp = fopen("result_basic_arith.dat", "w");
  ...
  for (i = 0; i < N; i++) {
      ...
      basic_arith(inA, inB, inC, inD, &out1, &out2, &out3, &out4);
...
//-              out3.to_int(), inD.to_int(), inA.to_int(), out4.to_int());
  ...
  retval = system("diff --brief -w result_basic_arith.dat result_basic_arith_ref.dat");
  if (retval != 0) {
      printf("Test failed  !!!\n");
      retval = 1;
  } else {
